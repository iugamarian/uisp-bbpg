...
source=(http://ftp.de.debian.org/debian/pool/main/u/${_pkgnameorig}/${_pkgnameorig}_${pkgver}.orig.tar.gz )
...
build() {
  ...
  cd "${srcdir}/extractpatches/${_pkgnameorig}-${pkgver}"
  patch -p1 -i ../uisp_20050207-4.2.diff
  cd "${srcdir}/${_pkgnameorig}-${pkgver}"
  patch -p1 -i ${srcdir}/extractpatches/${_pkgnameorig}-${pkgver}/debian/patches/10_const_char.dpatch || return 1
  patch -p1 -i ${srcdir}/extractpatches/${_pkgnameorig}-${pkgver}/debian/patches/20_fix_manpage.dpatch || return 1
  patch -p1 -i ${srcdir}/extractpatches/${_pkgnameorig}-${pkgver}/debian/patches/30_fix_g++_4.3.dpatch || return 1
  patch -p1 -i ${srcdir}/extractpatches/${_pkgnameorig}-${pkgver}/debian/patches/40_fix_g++_4.6.dpatch || return 1
  patch -p1 -i ${srcdir}/extractpatches/uisp-20050207-usb-bbpg-patch.txt || return 1
  sed -i 's/busses = usb_get_busses();/busses = usb_get_busses();\n  for (busses = usb_busses; busses; busses = busses->next)\n{\n}\n  busses = usb_get_busses();/g' ${srcdir}/${_pkgnameorig}-${pkgver}/src/ftdibb.c
